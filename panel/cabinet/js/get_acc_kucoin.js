...
function DrawUSDTBalance(Total, Available, Orders) {
    if (GetPageName() === "cabinet_accounts") {
        ...
        for (var i = 0; i < user_account.balance.balance_USDT.msg.length; i++) {
            ticker.push(user_account.balance.balance_USDT.msg[i].ticker)
            usdt_ticker.push(user_account.balance.balance_USDT.msg[i].available)
        }
        placePie(ticker, usdt_ticker);
    } else {
        document.querySelector(".validation-balance-block").style.display = "inline-block";
