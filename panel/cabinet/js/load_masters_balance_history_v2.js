...
function getGradient(ctx, chartArea, minY, maxY) {
    ...
    if (minY >= 5) {
        ...
        gradient.addColorStop(1, pointLowerColor);
            ...
            let fullDistance = maxY - minY;

            let upperDelta = maxY / fullDistance;
            let lowerDelta = Math.abs(minY / fullDistance);

            let zeroPoint = lowerDelta;
            let lowerHalfPoint = zeroPoint / 2
            let higherHalfPoint = zeroPoint + upperDelta / 2;
            gradient.addColorStop(zeroPoint, pointLowerColor);
            gradient.addColorStop(zeroPoint, pointUpperColor);
...
MasterBalanceHistoryLoader = class {
    ...
    DrawRoi = function (masterId, balancePoints) {
        ...
        let dailyState = "$" + parseInt(roiPercent * 100) + " (" + roiPercent + "%)";
        let weeklyState = "$" + parseInt(weeklyRoi * 100) + " (" + weeklyRoi + "%)";
        let weeklyState_x2 = "$" + parseInt(weeklyRoi_x2 * 100) + " (" + weeklyRoi_x2 + "%)";
        let weeklyState_x3 = "$" + parseInt(weeklyRoi_x3 * 100) + " (" + weeklyRoi_x3 + "%)";
        let fullState = "$" + parseInt(totalRoi * 100) + " (" + totalRoi + "%)";
        ...
        if (weeklyProfit_x2 >= 0) {
            ...
            roiTd.querySelector("[roi-per-week-x2]").style.color = lossColor;
    ...
    DrawROILine = (canvasId, balancePoints, masterId, displaying_days = 0) => {
        ...
        for (let i = startIndexROIBalancePoint + 1; i < balancePoints.length; i++) {
            ...
            let cumulativeTransfer = transfer_loader.GetSumTransferByMasterEmailFromToDay(masterEmail, currentPoint.date, startROIBalancePoint.date);
            let pointBalanceDelta = currentPoint.balance - startROIBalancePoint.balance;

            let pointBalanceRoi = ((pointBalanceDelta - cumulativeTransfer) / startROIBalancePoint.balance) * 100; //TODO include transfers
            ROILabels.push("")
