...
if (isset($_SESSION['user_email'])) {
    ...
    if ($user->isConnected()) {
//        $result = $conn->query("SELECT * FROM `binance_socket_log` WHERE `follower_email` = '".$user->user_email."' and `master_email` = '".$masterEmail."' ORDER BY `id` ASC");
        $result = $conn->query("SELECT * FROM `binance_socket_log` WHERE `follower_email` = '".$user->user_email."' ORDER BY `id` DESC");
